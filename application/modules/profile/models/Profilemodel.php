...
//echo $this->db->last_query(); die;
	...
	function getProfileOnly($ProfileId=NULL,$profile_status){
		$this->db->select('*');
		$this->db->from('profiles');
		...
		if(!empty($profile_status)){
			...
			$this->db->where("profile_status", "pending");
		}
	...
	function getAllProfileOnly($ProfileId=NULL){
		$this->db->select('*');
		$this->db->from('profiles');
		$this->db->order_by("profile_id", "DESC");
...
//		echo $this->db->last_query();
	...
	public function getProfileAdminsById($profileId=NULL){
		...
		$this->db->join('c_users b','b.user_id = a.c_user_id');
		$this->db->order_by("id", "DESC");
		if($profileId){
		$this->db->where("profile_id", $profileId);
		}
